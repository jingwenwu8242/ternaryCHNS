...
int nr, nz, n_level, p_relax, c_relax, it, frac;
...
int main()
{
    ...
    double **ou, **ow, **u, **w, **p, **np, **ophi1, **phi1, **nu, **nw, **nphi1, **ophi2, **phi2, **nphi2, **ophi3, T, mass1, mass2, res1, res2, red2, Pe, Cn, ht;
    ...
    n_level -= 0;
    rleft = 0.0, rright = 1;
    zleft = 0.0, zright = rright * (double)nz / (double)nr;
    ...
    vis1 = 1;
    vis2 = 1;
    vis3 = 100.0; // 8.90 �� 10 -4 Pa.s
    Re = 1.0;
    We = 1;
    Pe = 1.0 / 100;
    mobil = 1 / Pe; // 0.01 / gam;//0.01 / gam; // 0.01 / gam; // 0.01 / gam;
    sig = 1.0;
    frac = 0;
    lambda = 0.000;
    SS = 2.0;
    ...
    theta2 = theta * pi / 180.0; // 30.0*pi/180.0;
    kappa = 1e-8;
    /***********************/
    ...
    worku = dmatrix(0, nr, 1, nz); // grad_p=(worku,workw)//
    workw = dmatrix(1, nr, 0, nz);
    workp = dmatrix(1, nr, 1, nz); // workp :div_uw;used for calculate p//
    ...
    fclose(fphi2);
    fclose(fphi3);
    mymass1 = fopen("E:\\mass1.m", "w");
    // mymass2 = fopen("E:\\mass2.m", "w");
    mass1 = mass_comp(phi1);
    // mass2 = mass_comp(phi2);
    fprintf(mymass1, " %f\n", mass1);
    //  fprintf(mymass2, " %f\n", mass2);
    fclose(mymass1);
    //   fclose(mymass2);
    ...
    for (it = 1; it <= max_it; it++)
    {
        ...
        if (it % ns == 0)
        {
            ...
            my = fopen("E:\\remarks.m", "a");
            fprintf(my, "\n counts=    %d   iteration=    %d   CPU Time=   %f\n", count, it, elapsed);
            fclose(my);
            mass1 = mass_comp(phi1);
            //  mass2 = mass_comp(phi2);
            mymass1 = fopen("E:\\mass1.m", "a");
            //  mymass2 = fopen("E:\\mass2.m", "a");
            fprintf(mymass1, " %f\n", mass1);
            //  fprintf(mymass2, " %f\n", mass2);
            fclose(mymass1);
            //  fclose(mymass2);
...
void initialization(double **u, double **w, double **p, double **phi1, double **phi2, double **phi3)
{
    ...
    ikloop
    {
        ...
        phi3[i][k] = (1 - phi0) * (0.5 + 0.5 * tanh(-max(fabs(r - 0) - 0.8, fabs(z + 0) - 0.25) / (0.5 * 2 * sqrt(2.0) * gam)));
...
void cahn(double **c_old, double **cc_old, double **c_new, double **mu, double theta, double **adv_phi)
{
    ...
    while (it_mg <= max_it_CH && resid > tol)
    {
        ...
        relax_ch(c_new, Mo1, mu, sc, smu, 1, nr, nz);
...
void print_data(double **u, double **v, double **phi1, double **phi2, double **phi3, double **p)
{
    ...
    for (i = 1; i <= nr; i++)
    {
        for (k = 1; k <= nz; k++)
        {
            ...
            /*
                  fprintf(fu, " %f ", u[i-1][k]);
                  fprintf(fv, " %f ", v[i][k-1]);
            */
        }
        fprintf(fu, "\n ");
        fprintf(fv, "\n ");
